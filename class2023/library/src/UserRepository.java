...
public class UserRepository {
    ...
    public static int deleteMemberIndexSave;
    public static boolean checkTemp;
    public UserRepository() {
        //예비생성자
    }
    UserRepository(String memberName, int memberNumber, String memberAddress, int memberPhoneFrontNumber, int memberPhoneBackNumber, int[][] borrowBookList,int deleteMemberIndexSave, boolean checkTemp) {
        ...
        UserRepository.deleteMemberIndexSave = deleteMemberIndexSave;
    ...
    static void memberRegistrationName(Scanner sc, ArrayList<UserRepository> peopleInformation, int[][] borrowBookList) {
        ...
        if (deleteMemberIndexSave != 0) {
            System.out.println("새로운 회원번호는 "+deleteMemberIndexSave+"번 입니다");
        } else {
            deleteMemberIndexSave = (peopleInformation.size() + 1);
            System.out.println("새로운 회원번호는 "+deleteMemberIndexSave+"번 입니다");
        }
        System.out.println("회원의 이름을 적어주세요");
        String signupMemberName = sc.next();
        System.out.println("안내 : 회원 이름이 정상적으로 등록되었습니다");
        memberRegistrationAddress(sc, peopleInformation, borrowBookList, signupMemberName, deleteMemberIndexSave);
    }
    ...
    public static void memberPhoneBackNumber(Scanner sc, ArrayList<UserRepository> peopleInformation,int[][] borrowBookList,String signupMemberName,int signupMemberNumber,String signupMemberAddress,int signupFrontNumber) {
        while (true) {
            ...
            if (signupBackNumber >= 1000 && signupBackNumber <= 9999) {
                ...
                UserRepository success = new UserRepository(signupMemberName, signupMemberNumber, signupMemberAddress, signupFrontNumber, signupBackNumber, borrowBookList,deleteMemberIndexSave,checkTemp);
                peopleInformation.add(success);
                deleteMemberIndexSave = 0;
                break;
            } else {
                System.out.println("잘못된 입력입니다");
            }
    ...
    public static void memberDelete (Scanner sc,ArrayList<UserRepository> peopleInformation) {
        ...
        for (UserRepository memberIndex : peopleInformation) {
            if (memberIndex.memberNumber == memberNumber) {
                System.out.println("회원번호가 성공적으로 삭제되었습니다");
                deleteMemberIndexSave = memberNumber;
                checkTemp = true;
